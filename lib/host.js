...
GitServer.prototype.bindEvents = function(callback) {
  ...
  this.repos.forEach(function(repo) {
    ...
    repo.path = path.normalize(this.repoLocation+"/"+repo.name+".git");
    fs.emptyDir(repo.path + '/hooks', function() { 
        var my_git_events = git_events(repo.path);
        repo.last_commit = {};
        repo.event = function(repo, update) {
        ...
        var map = {
          "post-applypatch": repo.event,
          "post-commit": repo.event,
          "post-checkout": repo.event,
          "post-merge": repo.event,
          "post-receive": repo.event,
          "post-update": repo.event,
          "post-rewrite": repo.event,
          "applypatch-msg": repo.event,
          "pre-applypatch": repo.event,
          "pre-commit": repo.event,
          "prepare-commit-msg": repo.event,
          "commit-msg": repo.event,
          "pre-rebase": repo.event,
          "pre-receive": repo.event,
          "update": repo.event,
          "pre-auto-gc": repo.event
        }
        proxy(process, map, my_git_events, repo);
        repo.bound = true;
    });
  });
