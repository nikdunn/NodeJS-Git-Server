...
GitServer.prototype.bindEvents = function(callback) {
  ...
  for (var i in this.repos) {
    if (this.repos[i].bound) {
        continue;
    }
    this.repos[i].path = path.normalize(this.repoLocation+"/"+this.repos[i].name+".git");
    fs.emptyDir(this.repos[i].path + '/hooks', function() { 
        self.repos[i].git_events = git_events(self.repos[i].path);
        self.repos[i].last_commit = {};
        self.repos[i].event = function(repo, update) {
        ...
        var map = {
          "post-applypatch": self.repos[i].event,
          "post-commit": self.repos[i].event,
          "post-checkout": self.repos[i].event,
          "post-merge": self.repos[i].event,
          "post-receive": self.repos[i].event,
          "post-update": self.repos[i].event,
          "post-rewrite": self.repos[i].event,
          "applypatch-msg": self.repos[i].event,
          "pre-applypatch": self.repos[i].event,
          "pre-commit": self.repos[i].event,
          "prepare-commit-msg": self.repos[i].event,
          "commit-msg": self.repos[i].event,
          "pre-rebase": self.repos[i].event,
          "pre-receive": self.repos[i].event,
          "update": self.repos[i].event,
          "pre-auto-gc": self.repos[i].event
        }
        proxy(process, map, self.repos[i].git_events, self.repos[i]);
        self.repos[i].bound = true;
    });
  }
  if (callback)
    callback();
...
GitServer.prototype.onFetch = function(fetch) {
  ...
  if (repo !== false) {
    if (fetch.request.url.indexOf('receive-pack') > -1) {
